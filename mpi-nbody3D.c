/****************************************************************************
 *
 * mpi-nbody3D.c -- N-body simulation (from https://github.com/harrism/mini-nbody)
 *
...
{
	for (int i = 0; i < n; i++) {
		points[0][i] = randab(-1, 1);
		points[1][i] = randab(-1, 1);
		points[2][i] = randab(-1, 1);
		vpoints[0][i] = randab(-1, 1);
		vpoints[1][i] = randab(-1, 1);
		vpoints[2][i] = randab(-1, 1);
	}
}
				   ...
				   float dt, int n)
{
    for (int i = 0; i < n; i++) {
        ...
        for (int j = 0; j < n; j++) {
            const float dx = points[0][j] - points[0][i];
            const float dy = points[1][j] - points[1][i];
            const float dz = points[2][j] - points[2][i];
...
{
    for (int i = 0 ; i < n; i++) {
        points[0][i] += vpoints[0][i]*dt;
        points[1][i] += vpoints[1][i]*dt;
        points[2][i] += vpoints[2][i]*dt;
    }
}
              ...
              float *vpoints[], int n)
{
    ...
    */
    for (int i=0; i<n; i++) {
        e += 0.5 * (vpoints[0][i] * vpoints[0][i]) + (vpoints[1][i] * vpoints[1][i]) + (vpoints[2][i] * vpoints[2][i]);
         ...
         */
        for (int j=i+1; j<n; j++) {
            const float dx = points[0][i] - points[0][j];
            const float dy = points[1][i] - points[1][j];
            const float dz = points[2][i] - points[2][j];
...
int main(int argc, char* argv[])
{
    ...
    for (int i=0;i<3;i++) {
		points[i] = (float*)malloc(size); assert(points[i] != NULL);
		my_points[i] = (float*)malloc(my_size); assert(my_points[i] != NULL);
		vpoints[i] = (float*)malloc(size); assert(vpoints[i] != NULL);
		my_vpoints[i] = (float*)malloc(my_size); assert(my_vpoints[i] != NULL);
	}

	if ( 0 == my_rank ) {
		init(points, vpoints, nBodies);
	}
    ...
    for (int iter = 1; iter <= nIters; iter++) {
			...
			compute_force(my_points,my_vpoints, DT, my_nBodies);
			integrate_positions(my_points,my_vpoints, DT, my_nBodies);
			const double elapsed = hpc_gettime() - tstart;
			totalTime += elapsed;
			
			for (int i=0;i<DIMENSIONS;i++) {
				MPI_Allgather( my_points[i],	/* sendbuf      	 	 */
							   my_nBodies,		/* count; how many elements to send to _each_ destination */
							   MPI_FLOAT,		/* sent datatype 	 	 */
							   points[i],		/* recvbuf      	 	 */
							   my_nBodies,		/* recvcount    	 	 */
				...
				MPI_Allgather( my_vpoints[i],	/* sendbuf      	 	 */
							   my_nBodies,		/* count; how many elements to send to _each_ destination */
							   MPI_FLOAT,		/* sent datatype 	 	 */
							   vpoints[i],		/* recvbuf      	 	 */
							   my_nBodies,		/* recvcount    	 	 */
			...
			if ( 0 == my_rank ) {
				const float e = energy(points, vpoints, nBodies);
