...
The dot product $s$ of two arrays `x[]` and `y[]` is:

$$
s = \sum_{i=0}^{n-1} x[i] \times y[i]
$$

In this exercise we implement a simple (but not efficient) approach
where we use _one_ block of _BLKDIM_ threads. The algorithm works as
follows:

1. The GPU executes one thread block containing _BLKDIM_ threads; use
   the maximum number of threads per block supported by the hardware,
   which is _BLKDIM = 1024_.
